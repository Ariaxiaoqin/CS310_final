...
def lambda_handler(event, context):
  try:
    ...
    #
    print("**Writing local image file**")
    #
    ...
    with open(local_filename, "wb") as file:
            file.write(bytes)
    print("**Image written locally for processing**")
    resized_filename = "/tmp/resized_compressed.jpg"
    with Image.open(local_filename) as img:
            img = img.convert("RGB")  # Ensure it's in RGB format
            img.thumbnail((128, 128), Image.ANTIALIAS)  # Resize to 128x128
            img.save(resized_filename, "JPEG", quality=70) 
    ...
    bucketkey = "benfordapp/" + username + "/" + basename + "-" + str(uuid.uuid4()) + ".jpg"
    ...
    #
    bucket.upload_file(resized_filename, 
                      bucketkey, 
                      ExtraArgs={
                        'ACL': 'public-read',
